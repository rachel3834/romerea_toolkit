...
#get photometry data loaded in
with h5py.File(hdf5_path, "r") as f:
    dset = f["dataset_photometry"]
    data = dset[:]  # shape (stars, times)
...
#getting filters for all stars individually
qid_matrix = raw_data[:, :, qid_col]
filter_matrix = np.vectorize(lambda q: qid_to_filter.get(int(q), "unknown"))(qid_matrix)

#now using a loop over FILTERS
...
for flt in unique_filters:
    ...
    is_qc_good = raw_data[:, :, qc_flag_col] == 0
    valid = is_flt & is_qc_good

    #getting only valid magnitude values
    mag_data = np.where(valid, raw_data[:, :, mag_col], np.nan)
    ...
    valid_obj_ids = raw_data[:, 0, obj_id_col].astype(int)[valid_mask]
